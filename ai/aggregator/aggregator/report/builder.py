...
# ⚠️ **이 문자열은 그대로 LLM 에 들어간다.** 완성된 예시 문장을 쓰지 마라 — 모델이
# 그걸 실제 답변으로 복사한다. 세션 14 에서 발화 비율 41%(적게 말한 쪽)인 사용자에게
# "말수가 조금 많은 편이었어요" 가 나갔는데, 그 문장이 여기 예시에 그대로 있었다.
# 같은 이유로 이 주석에도 그 문장을 다시 넣으면 안 된다(주석은 프롬프트 밖이라 안전).
_OUTPUT_SPEC = """
## 출력 형식
...
{
  "summary": "<대화 전체 인상 한 문장>",
  "strengths": ["<잘한 점 1>", "<잘한 점 2>", "<잘한 점 3>"],
  "improvements": ["<개선점 1>", "<개선점 2>"],
  "missions": ["<다음에 시도할 행동 1>"],
  "cards": [
    {
      "kind": "issue",
      "title": "<지적 제목 (10자 내외)>",
      "context": "<그 장면 요약 한 문장>",
      "quoteRef": null,
      "suggestion": "<대신 해볼 말 한 문장>",
...
※ 꺾쇠 <> 안은 **자리표시자다. 형식만 보여준다.** 자리표시자를 그대로 옮기지도 말고,
   완성된 예시 문장을 지어내 채우지도 마라 — 반드시 아래 '측정 결과'와 '대화 기록'에서
   관찰된 사실로만 채워라.
